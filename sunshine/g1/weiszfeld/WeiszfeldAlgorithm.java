...
public class WeiszfeldAlgorithm{
    ...
    public static Output process(Input input) {
        ...
        for (Map.Entry<Point, Double> entry : map.entrySet()) {
            WeightedPoint wPoint = new WeightedPoint(entry.getValue(), entry.getKey());
            aPoints.add(wPoint);
        }
    ...
    private static Point operatorT(Point x, List<WeightedPoint> aPoints, int dimension) {
    ...
    private static Point operatorS(Point aj, double wj, Point rj
    ...
    private static Point R(Point aj, List<WeightedPoint> aPoints, int dimension) {
        Point result = new Point(dimension);
    ...
    private static double operatorL(Point aj, List<WeightedPoint> aPoints) {
        double res = 0;
        for (WeightedPoint ai: aPoints) {
     ...
     * @return 
     */
    private static double evaluateF(Point x, List<WeightedPoint> aPoints) {
        double res = 0;
        for (WeightedPoint ai: aPoints) {
